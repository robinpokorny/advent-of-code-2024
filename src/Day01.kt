...
private fun parse(input: List<String>): Pair<List<Int>, List<Int>> = input
    .map { it.split("\\s+".toRegex()).let { it[0].toInt() to it[1].toInt() } }
    .unzip()


private fun part1(input: Pair<List<Int>, List<Int>>): Int =

     input.first.sorted().zip(input.second.sorted()).sumOf { (x, y) -> abs(x - y) }
